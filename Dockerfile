...
RUN /bin/sh -c '$JBOSS_HOME/bin/standalone.sh &' && \
 ...
 $JBOSS_HOME/bin/jboss-cli.sh --connect --command='data-source add --jndi-name=java:/hotel --name=MySQLPool --connection-url=${DB_CONNECTION_URL:jdbc:mysql://localhost:3306/hotel} --driver-name=mysql --user-name=${DB_USERNAME:root} --password=${DB_PASSWORD:password}' &&\
